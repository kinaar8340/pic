...
class PlatonicHelicalGeometricNetwork(nn.Module):
    ...
    def forward(self, task_emb: torch.Tensor, level: PlatonicLevel, s: float = 12.0, pol: int = 0) -> torch.Tensor:
        ...
        # Video-frame topological memory — now stores TRUE 3D Clifford coordinate (global topology)
        # (quaternion + helical/Clifford geometry solve the AI persistent memory problem)
        geo_frame = self.conduit.get_helix_3d(float(s), pol).detach().cpu().clone()  # shape (3,)
        self.video_memory.append(geo_frame)
    ...
    def render_geometric_video(self, save_path: str = "outputs/pic_heartbeat_video.mp4"):
        ...
        Uses true 3D Clifford coordinate (global topology) + proper RGB scaling.
        Lazy cv2 + DRY normalization (configuration over hardcoding).
        """
        if not self.video_memory:
            print("⚠️ No Clifford frames in video_memory yet")
            return

        # Lazy import (SRP + optional dep)
        try:
            import cv2
        except ImportError:
            print("⚠️ OpenCV (cv2) not installed — skipping video render.")
            print("   (already installed in your env — this should not trigger)")
            return

        frames = []
        for f in self.video_memory:  # f is now true 3D Clifford coord (3,)
            xyz = f.numpy()
            # Affine map to visible RGB color pulse (preserves topological evolution)
            # (Clifford Torus skin + zero Gaussian curvature → meaningful color heartbeat)
            xyz_norm = ((xyz - xyz.min()) / (xyz.max() - xyz.min() + 1e-8) * 255.0).clip(0, 255)
            proj = xyz_norm.astype(np.uint8).reshape(1, 1, 3)
            proj = np.tile(proj, (256, 256, 1))  # solid color frame per Clifford position
            frames.append(proj)
        ...
        for f in frames:
            out.write(f)
        out.release()
        print(f"→ Geometric heartbeat video saved: {save_path} ({len(frames)} true Clifford frames)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PHGN — 3-Phase Star-Delta + Video Memory + Living Heartbeat (v9.9 Clifford)")
    ...
    print(f"❤️ Training PHGN v9.9 with Living Heartbeat — {len(facts)} facts | Clifford Torus Skin + 3-phase + video memory")
    ...
    for epoch in range(args.epochs):
        ...
        for idx, fact in enumerate(tqdm(facts, desc=f"Epoch {epoch}")):
            ...
            # ─── Global topology drives persistence (Clifford skin + braiding_phase) ───
            stats = conduit.monitor_topological_winding()
            braiding_phase = torch.tensor(
                stats.get('braiding_phase', 0.5),
                dtype=torch.float32,
                device=device
            )
            topo_loss = (braiding_phase - 0.5).pow(2) * 0.08
        ...
        if epoch % 20 == 0 or epoch == args.epochs - 1:
            print(f"Epoch {epoch:3d} | loss={total_loss/len(facts):.5f} | braiding_phase={stats.get('braiding_phase',0):.4f}")
